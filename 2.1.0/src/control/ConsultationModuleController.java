	...
	public Consultation findConsultationById(String id) {
		...
		return null;
            ...
            public boolean addDutySlot(String doctorId, java.time.DayOfWeek day, java.time.LocalTime start, java.time.LocalTime end) {
                entity.Doctor d = findDoctorById(doctorId);
                if (d == null || !start.isBefore(end)) return false;
                d.addDutySlot(new entity.DutySlot(day, start, end));
                return true;
            }
            public String listDoctors() {
                StringBuilder sb = new StringBuilder("=== Doctors ===\n");
                adt.MyIterator<entity.Doctor> it = doctors.iterator();
                while (it.hasNext()) {
            ...
            public String showSchedule(String doctorId) {
                ...
                if (d.getScheduleNumberOfEntries() == 0) {
                    sb.append("(no duty slots)\n");
                } else {
                    for (int i = 1; i <= d.getScheduleNumberOfEntries(); i++) {
                        sb.append("- ").append(d.getScheduleEntry(i)).append("\n");
                    }
                }
                return sb.toString();
            }
